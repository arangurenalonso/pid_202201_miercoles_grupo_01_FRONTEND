...
export class ListadoComponent implements OnInit {
  ...
  listar() {
    ...
    this.incidenteService.paginacion(filtroURL)
    .subscribe(response => {
      console.log(response)
      this.paginator = response.detalle.data;
      this.incidentes = response.detalle.data.contenido as Incidente[];
    });
  ...
  public changeActive(incidente: Incidente): void {
    let tipoObjeto:String="Incidente"
    Swal.fire({
      title: incidente.isActive? `Está seguro que desea DESACTIVAR el ${tipoObjeto}?`:`Está seguro que desea ACTIVAR el ${tipoObjeto}?`,
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        this.incidenteService.changeActive(incidente).subscribe(
          (response) => {
